...
module.exports = function (controller) {
	...
	function hello(bot, message) {
		bot.reply(message, "Xin chào, tôi là Real Estate Chatbot!");
		bot.reply(message, "Bạn cần tìm bất động sản như thế nào?")
	}
	...
	function getQuesion(attrName,attrValue,badAspect) {
		...
		if(badAspect == 'none'){
			if(attrName=='addr_street' ||
				...
				attrName=='addr_ward'|| 
				attrName=='addr_city'){
				question.text ='Bạn có muốn bất động sản này ở địa chỉ:'
		}
		else if(attrName=='surrounding'||
			attrName=='surrounding_name'||
			attrName=='surrounding_characteristics'){
			question.text = 'Bạn có muốn khu vực xung quanh hay gần bất động sản cần tìm là:'
	}
	else if(attrName=='interior_room'||
		attrName=='interior_floor'||
		attrName=='legal'){
		question.text = 'Bạn muốn có bất động sản cần tìm có:'
}
else if(attrName=='orientation'){
	question.text ='Bạn có muốn bất động sản có hướng là:'
}
else if(attrName=='area'){
	question.text ='Bạn có muốn bất động sản có kích trước/ diện tích là:'
}
else if(attrName=='transaction_type'){
	question.text ='Giao dịch bạn cần tìm là:'
}
else if(attrName=='realestate_type'){
	question.text ='Bất động sản bạn cần tìm là:'
}
else if(attrName=='price'){
	question.text ='Bạn có muốn tìm bất động sản có giá là:'
}
else if(attrName=='position'){
	question.text ='Bạn có muốn bất động sản có chỗ ra vào ở:'    
}
else if(attrName=='potential'){
	question.text ='Bạn có muốn dùng bất động sản này để:'    
}
else if(attrName=='project'){
	question.text ='Bạn có muốn bất động sản thuộc dự án:'    
}
else {
...
async function query(input, message, isText = true) {
  ...
  controller.hears('Ngừng xem kết quả', 'message_received', function (bot, message) {
  	bot.startConversation(message, function (err, convo) {
  		convo.say({
  			text: 'Bạn muốn chỉnh sửa yêu cầu tìm kiếm gì?',
  			quick_replies: [
  			{
  				title: 'Thêm yêu cầu',
